...
QString Cryptography::clientXhash = "";
QString Cryptography::clienthash = "";
QString Cryptography::version = "";
QString Cryptography::gfuid = "";
QString Cryptography::loginPacket = "";
...
void Cryptography::setGFuid(const QString &value)
{
    gfuid = value;
}

void Cryptography::setClientXhash(QString hash)
{
    //QDir d;
    clientXhash = hash;//fileChecksum(d.absolutePath() + "/NostaleClientX.exe");
    qDebug() << "NostaleClientX.exe hash : " << clientXhash;
}

void Cryptography::setClienthash(QString hash)
{
    //QDir d;
    clienthash = hash;//fileChecksum(d.absolutePath() + "/NostaleClient.exe");
    qDebug() << "NostaleClient.exe hash : " << clienthash;
}

QString Cryptography::fileChecksum(const QString &fileName)
{
    QFile f(fileName);
...
void Cryptography::setVersion(QString hash)
{
    version = hash;
    /*QDir d;
    QString fName = d.absolutePath() + "/NostaleClientX.exe";
    qDebug() << fName;
    DWORD dwHandle;
    ...
    qDebug() << "NosTale version : " << version;
}
...
QString Cryptography::getLoginPacket()
{
    return loginPacket;
}
...
QByteArray Cryptography::createLoginPacket(const QString& username, const QString& password, const QString& langCode) const
{
    QByteArray output;
    std::vector<unsigned char> temp;
    CodeGenerator codeGen(username, password, langCode, gfuid, nullptr);
    ...
    loginPacket = beginn + "  " + gfuid + " 002ADCCF " + langCode + version + " 0 " + loginhash;
    qDebug() << loginPacket;
